...
#define FLASH_BASE_ADDRESS  ((uint32_t)0x0800FC00)
...
{
  eeprom_buffer_fill();
  uint16_t temp = eeprom_buffered_read_byte(pos) + (eeprom_buffered_read_byte(pos+1) << 8);
  return temp;
}

void eeprom_write_half_word(uint32_t pos, uint16_t value)
{
  eeprom_buffered_write_byte(pos, (uint8_t)value);
  eeprom_buffered_write_byte(pos+1, (uint8_t)(value >> 8));
  eeprom_buffer_flush();
}

void eeprom_update_byte(uint32_t pos, uint8_t value)
{
  if (eeprom_read_byte(pos) != value)
  {
    eeprom_write_byte(pos, value);
  }
}

void eeprom_update_half_word(uint32_t pos, uint16_t value)
{
  if (eeprom_read_half_word(pos) != value)
  {
    eeprom_write_half_word(pos, value);
  }
}


//! @brief Erase whole EEPROM
void eerom_erase_all()
{
  FLASH_EraseInitTypeDef EraseInitStruct;
  uint32_t pageError = 0;
  EraseInitStruct.TypeErase = FLASH_TYPEERASE_PAGES;
  EraseInitStruct.NbPages = (FLASH_BANK1_END - FLASH_BASE_ADDRESS + 1) / FLASH_PAGE_SIZE;
...
{
  if (eeprom_read_byte(E2END) != layoutVersion)
    eerom_erase_all();
}
...
{
  if (filament < ARR_SIZE(eeprom_t::eepromBowdenLen))
    return true;
  else
    return false;
}
...
uint16_t BowdenLength::get()
{
  ...
  {
    uint16_t bowdenLength = eeprom_read_half_word((uint32_t) & (eepromBase->eepromBowdenLen[filament]));
...
BowdenLength::~BowdenLength()
{
  if (validFilament(m_filament))
    eeprom_update_half_word((uint32_t) & (eepromBase->eepromBowdenLen[m_filament]), m_length);
}
...
bool FilamentLoaded::setStatus(uint8_t status)
{
  ...
  {
    eeprom_update_byte((uint32_t) & (eepromBase->eepromFilamentStatus[i]), status);
  }
  if (getStatus() == status)
    return true;
  return false;
...
int16_t FilamentLoaded::getIndex()
{
  ...
  case KeyFront2:
    ...
    {
      if (status != (eeprom_read_byte((uint32_t) & (eepromBase->eepromFilament[i])) >> 4))
      {
        index = i - 1;
        break;
      }
...
bool FilamentLoaded::get(uint8_t &filament)
{
  ...
  if ((index < 0) || (static_cast<uint16_t>(index) >= ARR_SIZE(eeprom_t::eepromFilament)))
    return false;
  const uint8_t rawFilament = eeprom_read_byte((uint32_t) & (eepromBase->eepromFilament[index]));
...
bool FilamentLoaded::set(uint8_t filament)
{
  for (uint8_t i = 0; i < BehindLastKey - 1; ++i)
  {
    ...
    eeprom_update_byte((uint32_t) & (eepromBase->eepromFilament[index]), filamentRaw);
    if (filamentRaw == eeprom_read_byte((uint32_t) & (eepromBase->eepromFilament[index])))
      return true;
    getNext(status);
    if (!setStatus(status))
      return false;
...
{
  uint8_t first = eeprom_read_byte((uint32_t) & (eepromBase->eepromDriveErrorCountL[0]));
  uint8_t second = eeprom_read_byte((uint32_t) & (eepromBase->eepromDriveErrorCountL[1]));
...
{
  eeprom_update_byte((uint32_t) & (eepromBase->eepromDriveErrorCountL[lowByte % 2]), lowByte - 1);
}

uint8_t DriveError::getH()
{
  return (eeprom_read_byte((uint32_t) & (eepromBase->eepromDriveErrorCountH)) + 1);
}

void DriveError::setH(uint8_t highByte)
{
  eeprom_update_byte((uint32_t) & (eepromBase->eepromDriveErrorCountH), highByte - 1);
}
